...
startup {
  ...
  settings.Add("Always start the splits on movement", false);
...
init {
  ...
  vars.GetPlayerPosition = (Func<Tuple<double, double, double>>) (() => {
    ...
    double playerZ = Math.Round(new DeepPointer(vars.playerPos + 0x08).Deref<float>(game), 3);
    return new Tuple<double, double, double>(playerX, playerY, playerZ);
  });
