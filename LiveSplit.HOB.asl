...
startup {
  ...
  var levelNames = new Dictionary<string, string> {
    ...
    // Not used in the splits but documenting here in case we need it later
    { "SPIN ROOM",  "MEDIA/LEVELS/DUNGEONS/FOREST DUNGEONS/DUNGEON_MOTHERHOB.LAYOUT" },
    { "ROCKET LAUNCHER", "MEDIA/LEVELS/DUNGEONS/OVERWORLD/8_5_LAKEDUNG.LAYOUT" },
    { "RED CLOAK", "MEDIA/LEVELS/DUNGEONS/OUTFITDUNGEONS/RED_OUTFIT_DUNGEON.LAYOUT" },
    { "GREEN CLOAK", "MEDIA/LEVELS/DUNGEONS/OUTFITDUNGEONS/GREEN_OUTFIT_DUNGEON.LAYOUT" },
    { "PURPLE CLOAK", "MEDIA/LEVELS/DUNGEONS/OUTFITDUNGEONS/PURPLE_OUTFIT_DUNGEON.LAYOUT" },
    { "POST-ELECTRIC", "MEDIA/LEVELS/DUNGEONS/OVERWORLD/10_10_FORESTTOTOWER.LAYOUT" },
  };
  ...
  var animationNames = new Dictionary<string, string> {
    ...
    // INT_TITAN_CORE_INSERT_FIRST
    // INT_TITAN_CORE_INSERT_SECOND
    // INT_TITAN_CORE_INSERT_FINAL
    ...
    { "HANDLE", "INT_HANDLE_TURN" },
    { "TABLET_GET", "CINE_WORLDPIECE_WATER_PICKUP" },
    { "TABLET_PUT", "CINE_WORLDPIECE_FOREST_INSERT" },
  ...
  vars.StartedAnimation = (Func<string, bool>)((string animation) => {
    animation = animationNames[animation];
    return vars.moveset.Changed && vars.moveset.Current == 16 && (vars.animation.Current == animation || vars.animation2.Current == animation);
  });
  ...
  var addAnimationSetting = (Action<double, double, double, string, string, string, string>)((double x, double y, double z, string animation, string id, string text, string tooltip) => {
    settings.Add(id, false, text);
    settings.SetToolTip(id, tooltip);
    vars.splits[id] = (Func<bool>)(() => vars.StartedAnimation(animation) && vars.CloseToPoint(x, y, z));
  });
  ...
  addAnimationSetting(24.2, 4, -246.9, "HANDLE",        "electric_lever",   "Activate Spider",      "Powering up the spider in the electrical area");
  ...
  addAnimationSetting(141.5, -118.4, 10.5, "BUTTON",    "raise_factory",    "Factory Handprint",    "Pressing the wall handprint in factory to raise the land");
  addExitLevelSetting("FACTORY",                        "exit_factory",     "Exit Factory",         "Exiting the factory after grabbing the forest tablet");
  addAnimationSetting(181.0, 0.8, -361.6, "TABLET_PUT", "place_forest",     "Activate Forest",      "Placing the forest tablet to raise the land");
  addAnimationSetting(257, 25.7, -254.5, "CORRUPTION",  "forest_corrupt",   "Forest Corruption",    "Clearing the corruption in the forest");
  addAnimationSetting(-17.5, 8.2, -111, "CORRUPTION",   "electric_corrupt", "Electric Corruption",  "Clearing the corruption in the electrical area");
  addAnimationSetting(-102.9, 0.8, -38.4, "TABLET_PUT", "place_cemetery",   "Activate Cemetery",    "Placing the cemetery tablet to raise the land");
  addAnimationSetting(-11.6, 6, 69, "CORRUPTION",       "cemetery_corrupt", "Cemetery Corruption",  "Clearing the corruption in the cemetery");
  ...
  addAnimationSetting(-45.9, 14, 123.5, "TABLET_GET",   "grab_water",       "Water Tablet",         "Pikcing up the water tablet");
  addAnimationSetting(-317.5, 10.8, 86.3, "TABLET_PUT", "place_water",      "Activate Water",       "Placing the water tablet to raise the land");
  addEnterLevelSetting("UNDERWATER",                    "enter_underwater", "Enter Underwater",     "Entering the underwater dungeon (only splits once)");
  addAnimationSetting(-126.3, 0, -36.8, "CORRUPTION",   "water_corrupt",    "Water Corruption",     "Clearing the corruption underwater");
  ...
  addAnimationSetting(-338.1, 6, 18.9, "CORRUPTION",    "pipes_corrupt",    "Pipes Corruption",     "Clearing the corruption in the pipes area");
  addAnimationSetting(-78.3, 1, -383.3, "CORRUPTION",   "intro_corrupt",    "Intro Corruption",     "Clearing the corruption in the introduction area");
  addAnimationSetting(-338.4, -2, -282.1, "CORRUPTION", "wetlands_corrupt", "Wetlands Corruption",  "Clearing the corruption in the wetlands");
  addAnimationSetting(-372.4, -7.1, -304.2, "CORE",     "wetlands_core",    "Wetlands Core",        "Picking up the core from the colossus in wetlands");
  addAnimationSetting(256.3, 25.7, -252.8, "CORE",      "sprite_core",      "Sprite Mom Core",      "Picking up the core from Sprite Mom in forest");
  addAnimationSetting(299, 13, -145, "CORE",            "forest_core",      "Forest Core",          "Picking up the core from the colossus in forest");
  addAnimationSetting(51, 1.6, 98.4, "BUTTON",          "robot_wakeup",     "Activate Colossus",    "Pressing the handprint after inserting all 3 cores");

  settings.CurrentDefaultParent = null;
  settings.Add("miscSplits", true, "Miscellaneous splits");
  settings.CurrentDefaultParent = "miscSplits";

  settings.Add("butterflies", false, "Split when collecting any butterfly");
...
split {
  ...
  if (settings["butterflies"] && vars.StartedAnimation("BUTTERFLY")) {
    vars.log("Collected butterfly at " + vars.hobX.Current + " " + vars.hobY.Current + " " + vars.hobZ.Current);
    return true;
  }

  if (settings["swords"] && vars.StartedAnimation("SWORD")) {
    vars.log("Collected sword piece at " + vars.hobX.Current + " " + vars.hobY.Current + " " + vars.hobZ.Current);
    return true;
  }

  if (settings["hearts"] && vars.StartedAnimation("HEALTH")) {
    vars.log("Collected health piece at " + vars.hobX.Current + " " + vars.hobY.Current + " " + vars.hobZ.Current);
    return true;
  }
  ...
  // TODO: It seems like, in some cases there are multiple animations in the final cutscene.
  // I added an 'unsplit' to the update block in case a second animation starts playing, but I should *probably* find the animation name/id to make this more robust.
  if (vars.moveset.Changed && vars.moveset.Old == 16 && vars.CloseToPoint(0.4, 126, 102.3)) {
  ...
  if (!vars.completedSplits.Contains("bad_ending") && vars.completedSplits.Contains("robot_wakeup") && vars.CloseToPoint(83.5, 0, -92.3)) {
    vars.log("Backup split for 'end of game' (e.g. good ending)");
    return true;
  }

  if (vars.moveset.Changed && vars.moveset.Current == 16) {
    vars.log("Started an unhandled animation at " + vars.hobX.Current + " " + vars.hobY.Current + " " + vars.hobZ.Current + " 1: '" + vars.animation.Current + "' 2: '" + vars.animation2.Current + "'");
  }
  if (vars.level.Changed) {
    vars.log("Unhandled level change from '" + vars.level.Old + "' to '" + vars.level.Current + "'");
  }
